...
import os
os.environ['CUDA_VISIBLE_DEVICES'] = "1"#必须再import torch之前
import time
import math
import socket
import argparse
...
#查看模型网络结构：https://blog.csdn.net/caicaiatnbu/article/details/89001837
import torchvision.models as models
from torchsummary import summary


parser = argparse.ArgumentParser()
parser.add_argument('--phase', default='train', help='train or test [default: test]')
...
parser.add_argument('--model', default='model_res_mesh_pool', help='Model name [default: networks]')
parser.add_argument('--log_dir', default='model/new', help='Log dir')
...
# parser.add_argument('--dataset', default='F:/syz/DW/PYDW/name_dongweiming/file/upsample/point-normals-upsampling-master/data/Patches_noHole_and_collected.h5')#default=None
...
parser.add_argument('--FWWD', default=0.0001, action='store_true',
                    help="move WD loss cal in g forward (for memory balance)")#default=False
...
parser.add_argument('--num_workers_each_gpu', type=int, default=3, help='[default: 4]')#default=4
# parser.add_argument('--num_workers_each_gpu', type=int, default=0, help='[default: 4]')
...
device = torch.device('cuda:{}'.format(int(0)) if torch.cuda.is_available() else 'cpu')#可见的GPU只有上面设定的，默认从0开始
...
def load_checkpoint(model, optimizer, fc_optimizer=None, name='g'):
    ...
    if os.path.isfile(_file):
        try:
            checkpoint = torch.load(_file,map_location='cpu')
...
def train(assign_model_path=None, bn_decay=0.95):
    torch.backends.cudnn.benchmark = False#网络加速。开始时搜索最适合的卷积实现算法，适合卷积不变的时候进行加速
    learning_rate = BASE_LEARNING_RATE

    try:
        os.makedirs(MODEL_DIR)#创建log文件夹
        os.makedirs(os.path.join(MODEL_DIR, 'train'))
    except:
        print('log dir is not empty!!!!!!!')

    log_writer = SummaryWriter()#日志
    g_learning_rate = learning_rate #[0]
    LDset = data_loader.LoaderDataset(FLAGS.dataset, BATCH_SIZE, USE_DATA_NORM, [0], transforms=None)#GPU[0]实际没用到
    ...
    #参数’fc‘
    # fc_parameters = list(filter(lambda kv: 'fc' in kv[0], g_model.named_parameters()))#filter(a,b)过滤b中符合a条件的。lambda kv: 'fc' in kv[0]，匿名函数，kv[0]='fc'
    #不是’fc‘是’conv‘
    ...
    if ASSIGN_MODEL_PATH > 0:
        ...
        g_model, g_optimizer, g_fc_optimizer = load_checkpoint(g_model, g_optimizer, 'g')
    ...
    for epoch in tqdm(range(restore_epoch, MAX_EPOCH + 1), ncols=55):
        ...
        for i_batch, sample_batched in enumerate(tqdm(dloader)):#Tqdm 进度条提示，i_batch是索引号，sample_batched是个len=4的列表
            ...
            if not FLAGS.FWWD:
                ...
                # pointclouds_pl, pointclouds_gt, radius = pointclouds_pl[0].to("cuda:1"), pointclouds_gt[0].to("cuda:1"), radius[0].to("cuda:1")
                pred, _, WD_loss, reg_loss, uniform_loss, repulsion_loss, _ = g_model(pointclouds_pl, WD,
                                                                                      pointclouds_gt,
                                                                                      this_scale=this_scale)#[8,gt_n,3]
            ...
            pre_gen_loss = WD_loss + uniform_loss * (1e-3) + repulsion_loss * (5e-3)
...
if __name__ == "__main__":
    if Replace == True:
        try:
            import shutil
            shutil.rmtree(os.path.join(MODEL_DIR, 'code/'))#shutil.rmtree()递归地删除文件
        except:
            pass
    if PHASE == 'train':
        ...
        print("log路径存在")
        train(assign_model_path=ASSIGN_MODEL_PATH)#参数为中断的epoch
    else:
