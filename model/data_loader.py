...
def load_patch_data(h5_filename, skip_rate=1, use_randominput=True, norm=False):
    if use_randominput:
        ...
        input = f['poisson_4096'][:]#input=(4000, 4096, 6)
        gt = f['poisson_4096'][:]#gt=(4000, 4096, 6)
    else:
    ...
    name = [str(item) for item in f['name'][:]]#len(name)=4000
    ...
    input = input[::skip_rate]#每隔skip_rate的长度取值
    gt = gt[::skip_rate]
...
class LoaderDataset(data.Dataset):
    def __init__(self, dataset, batch_size, use_norm, gpu_ids=0, transforms=None, train=True, refinenet=False):
        super().__init__()
        ...
        self.sample_cnt = self.input_data.shape[0]#模型个数
        self.num_batches = self.sample_cnt // self.batch_size#每个batch中的模型数量。//表示整数除法
    ...
    def __getitem__(self, idx):
        if not self.refinenet:#refine 完善网络
            scale_idx = np.random.randint(self.len_scale)
            this_scale = self.scale[scale_idx]#随机取scale X1-X16
            num_to_exclude = 0
            gt_npoint_to_sample = self.gt_data.shape[1]-num_to_exclude#每个模型4096个点
            npoint_to_sample = int(gt_npoint_to_sample / this_scale)
            gt_npoint_to_sample = int(npoint_to_sample * this_scale)#应该是保证是scale的倍数，由4096变为4087

            #把每个batch的输入数据进行shuffle
            if self.cnt % self.num_batches == 0:#num_batches每个batch中模型数量
                idx = np.arange(self.sample_cnt)#sample_cnt模型数量
            ...
            batch_input_data = self.input_data[start_idx:end_idx, :, :].copy()  # b,n,6 [24,4096,6]
            batch_data_gt = self.gt_data[start_idx:end_idx, :, :].copy()  # b,n,6
            radius = self.radius_data[start_idx:end_idx].copy()  # b

            self.cnt += 1
            batch_data_gt = batch_data_gt[:,0:gt_npoint_to_sample,:]#取前4087个点

            assert batch_data_gt.shape[1] == gt_npoint_to_sample
            new_batch_input = np.zeros((self.batch_size, npoint_to_sample, batch_input_data.shape[2]))#初始化一个[24,312,6][B,n,c]
            #对上文随机生成的采样率，在gt中非均匀采样出相应数量的输入
            if npoint_to_sample <= int(batch_input_data.shape[1]/2):#低于2倍降采样
            ...
            #数据增强（旋转、缩放、偏移、加位移噪声、加旋转噪声）
            if self.use_norm:#使用归一化，这里是归一化之后的结果
            ...
            # print("输入大小={}，gt={}，r={}，尺寸={}".format(batch_input_data.shape,batch_data_gt.shape,radius.shape,this_scale.shape))
            ...
            return batch_input_data, batch_data_gt #返回值len(tuple)=4,[24, 274, 6],[24, 4082, 6],[24],[],274和4082会变
...
if __name__ == '__main__':
    dataset='F:/syz/DW/PYDW/name_dongweiming/file/upsample/point-normals-upsampling-master/data/Patches_noHole_and_collected.h5'
    LDset = LoaderDataset(dataset, 24, True, [0], transforms=None)
    a=LDset[0]
    print("完毕")
    print(len(a))
    for i in range(4):
