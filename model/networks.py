...
def knn_point(k, xyz1, xyz2):
    ...
    """
    xyz1 = torch.unsqueeze(xyz1, 1)#[8,1,n,3]
    xyz2 = torch.unsqueeze(xyz2, 2)#[8,n,1,3]
    xyz1 = xyz1-xyz2#[8,n,n,3]
    del xyz2 #用del一遍计算一边清除中间变量。减小显存消耗，优化显存使用，避免out of memory
    # #因为超内存了，所以释放无关内存
    # if hasattr(torch.cuda, 'empty_cache'):
    #     torch.cuda.empty_cache()

    dist = torch.sum(xyz1 ** 2, -1)#[8,n,n]
    del xyz1
    val, idx = torch.topk(-dist, k=k)#[8,n,9
    del dist
    # print("K={},k近邻={}".format(k,idx.shape))
    return val, idx.int()


def group(xyz, points, k, dilation=1, use_xyz=False):#求xyz的k近邻。dilation扩张
    _, idx = knn_point(k * dilation + 1, xyz, xyz)#idx.shape=[8,n,9][B,N,C]
    idx = idx[:, :, 1::dilation].contiguous()#dix=[8,n,8],dilation表示步长。contiguous()物理存储时也连续
    # print("idx={}".format(idx.shape))
    # print("完毕")
    xyz_trans = xyz.transpose(1, 2).contiguous() #[8,3,n]
    ...
    if grouped_xyz.shape[1] != xyz.shape[1] or idx.shape[1] != xyz.shape[1]:
        pass
    del xyz_trans
    grouped_xyz = grouped_xyz.permute(0, 2, 3, 1).contiguous()#[8,n,8,3]

    xyz_trans = torch.unsqueeze(xyz, 2)#[8,n,1,3]

    grouped_xyz -= xyz_trans#[8,n,8,3]各坐标的差值
    del xyz_trans
    ...
    if grouped_points.shape[1] != xyz.shape[1] or idx.shape[1] != xyz.shape[1]:
        pass
    #grouped_xyz[8,n,8,3]各坐标的差
    #grouped_points[8,n,8,3]
    #idx[8,n,8] k邻域的编号
    return grouped_xyz, grouped_points, idx


class pointcnn(nn.Module):
    def __init__(self, k, n_cout, n_blocks, bn_decay=None, activation=nn.ReLU(inplace=True),device=None):
    ...
    def forward(self, xyz, use_bn=True, use_ibn=False, activation=nn.ReLU(inplace=True)):
        ...
        """
        _, grouped_points, _ = group(xyz.detach(), None, self.npoint)#.detach()切断一些分支的反向传播
        grouped_points = grouped_points.permute(0, 3, 1, 2).contiguous()#[8,3,n,8]
...
class res_gcn_up(nn.Module):
    def __init__(self, k, n_cout, n_blocks, bn_decay=None, indices=None, up_ratio=2, use_bn=False, multi_gpus=False,
        ...
        self.k = k
        self.n_blocks = n_blocks #9
        self.indices = indices #None
        self.up_ratio = up_ratio #2
        self.use_bn = use_bn
        self.multi_gpus = multi_gpus
        self.device = device
    ...
    def forward(self, xyz, points, up_scale, mask, Nout=0, indices=None):
        ...
        features = points
        if Nout == 0:
            Nout = int(xyz.shape[1]*up_scale)#n*R
        ...
        features = self.conv1(center_points)
        del center_points
        grouped_points_nn = self.conv2(grouped_points)#对局部进行1*1卷积[8,128,n,8]
        del grouped_points
        features = torch.cat([features, grouped_points_nn], 3)#[8,128,n,9]
        features = torch.mean(features, 3, keepdim=False) + shortcut#[8,128,n]
        ...
        #修改，删掉metablock
        # reg_loss=torch.zeros(1,out=None)+(1e-6)
        reg_loss=np.zeros(1)
        # print("reg_loss={}".format(type(reg_loss)))
...
def get_uniform_loss(pcd, percentages=[0.004,0.008,0.010,0.012,0.016], radius=1.0, device=None):
    N = pcd.shape[1]
    npoint = int(N * 0.05)
    loss=torch.zeros([1], requires_grad=True).cuda(non_blocking=True, device=device)
...
class GenModel(nn.Module):
    ...
    def forward(self, point_cloud, WD=None, pointclouds_gt=None, this_scale=4):
        ...
        else:#不使用法向，xyz只存了坐标[8,n,3]
            points = self.pointcnn(xyz, use_bn=self.use_bn)#[8,128,n]
        inN = xyz.shape[1]#输入的n
        first_up_scale = math.sqrt(this_scale)+(1e-06)
        outN1 = int(inN * round(first_up_scale, 3))#round将输入input张量每个元素舍入到最近,保留三位小数，结果是3.66
        try:
        ...
        if self.training:
            uniform_loss = get_uniform_loss(new_xyz, device=self.device)
